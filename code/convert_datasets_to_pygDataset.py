...
class dataset_Hypergraph2(InMemoryDataset):
    def __init__(self, root = '../data/pyg_data/hypergraph_dataset_updated/', name = None, 
                 p2raw = None,
                 train_percent = 0.01,
                 feature_noise = None,
        ...
        self.root = root
        self.myraw_dir = osp.join(root, self.full_name, 'raw')
        self.myprocessed_dir = osp.join(root, self.full_name)
        
        super(dataset_Hypergraph2, self).__init__(osp.join(root, full_name), transform, pre_transform)
    ...
    def raw_file_names(self):
        if self.feature_noise is not None:
            file_names = [f'{self.full_name}_noise_{self.feature_noise}']
        else:
            file_names = [self.name]
        return file_names
